...
export default function Home() {
  ...
  const [approvalIntent, setApprovalIntent] = useState<'email' | 'application' | 'all' | null>(null);
  ...
  async function approveAndRun() {
    ...
    try {
      const response = await fetch('/api/approvals', {
        ...
        body: JSON.stringify({ action: 'approve_ready_and_run', itemType: approvalIntent, enableAutomatic }),
      });
      const payload = await response.json() as { error?: string; emailCount?: number; applicationCount?: number; automaticEnabled?: boolean };
      if (!response.ok) throw new Error(payload.error ?? 'The approval could not be recorded.');
      setNotice(`${payload.emailCount ?? 0} email(s) and ${payload.applicationCount ?? 0} application(s) approved and queued.${payload.automaticEnabled ? ' Automatic daily execution is now enabled.' : ''}`);
      setApprovalIntent(null); setEnableAutomatic(false); setView('Runs'); await refresh(true);
    } catch (reason) {
  ...
  return (
    <main className="app-shell">
      ...
      {data && approvalIntent ? <ApprovalModal items={data.approvals.filter((item) => item.status === 'pending' && item.readiness === 'ready' && (approvalIntent === 'all' || item.item_type === approvalIntent))} scope={approvalIntent} busy={busy} enableAutomatic={enableAutomatic} onAutomatic={setEnableAutomatic} onCancel={() => { setApprovalIntent(null); setEnableAutomatic(false); }} onConfirm={() => void approveAndRun()} /> : null}
    </main>
  );
}
...
function ApprovalsView({ data, busy, onApprove }: { data: DashboardData; busy: boolean; onApprove: (scope: 'email' | 'application' | 'all') => void }) {
  ...
  return <>
    ...
    <section className="approval-toolbar panel"><div><strong>{pending.length} awaiting decision</strong><span>{readyEmails + readyApplications} ready · {pending.length - readyEmails - readyApplications} blocked</span></div><div><button className="secondary-ink-button" disabled={busy || readyEmails === 0} onClick={() => onApprove('email')} type="button">Approve {readyEmails} ready email{readyEmails === 1 ? '' : 's'}</button><button className="secondary-ink-button" disabled={busy || readyApplications === 0} onClick={() => onApprove('application')} type="button">Approve {readyApplications} ready application{readyApplications === 1 ? '' : 's'}</button><button className="primary-button" disabled={busy || readyEmails + readyApplications === 0} onClick={() => onApprove('all')} type="button"><span>✓</span>Approve ready &amp; run</button></div></section>
...
function ApprovalModal({ items, scope, busy, enableAutomatic, onAutomatic, onCancel, onConfirm }: { items: ApprovalItem[]; scope: 'email' | 'application' | 'all'; busy: boolean; enableAutomatic: boolean; onAutomatic: (value: boolean) => void; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) onCancel(); }}><section aria-labelledby="approval-title" aria-modal="true" className="modal" role="dialog"><div className="modal-head"><div><span className="eyebrow">Final authorization</span><h2 id="approval-title">Approve and queue this batch?</h2></div><button aria-label="Close" onClick={onCancel} type="button">×</button></div><div className="modal-warning"><strong>This click records your action-time approval.</strong><span>The worker may send the listed emails or submit the listed applications. Blocked items are excluded.</span></div><dl className="modal-facts"><div><dt>Emails</dt><dd>{emails}</dd></div><div><dt>Applications</dt><dd>{applications}</dd></div><div><dt>Scope</dt><dd>{humanStatus(scope)}</dd></div></dl><div className="modal-item-list">{items.map((item) => <div key={item.id}><strong>{item.subject}</strong><span>{item.target || item.company}</span></div>)}</div><label className="automatic-option"><input checked={enableAutomatic} onChange={(event) => onAutomatic(event.target.checked)} type="checkbox" /><span><strong>Enable automatic daily execution after this batch</strong><small>Future truth-checked items may run at 9:00 PM within the saved caps. Replies still pause follow-ups and notify you.</small></span></label><div className="modal-actions"><button className="secondary-ink-button" disabled={busy} onClick={onCancel} type="button">Cancel</button><button className="primary-button" disabled={busy || items.length === 0} onClick={onConfirm} type="button"><span>✓</span>{busy ? 'Authorizing…' : 'Approve & queue execution'}</button></div></section></div>;
}
...
function SettingsView({ settings, missing, onSaved }: { settings: Settings; missing: string[]; onSaved: () => Promise<void> }) {
  ...
  return <form className="settings-form" onSubmit={submit}>
    ...
    <section className="panel settings-section"><PanelHeading eyebrow="Run controls" title="Volume and schedule" note="Conservative starting caps" /><div className="form-grid four-fields"><Field label="Mode"><select value={form.automation_mode} onChange={(event) => update('automation_mode', event.target.value as Settings['automation_mode'])}><option value="paused">Paused</option><option value="manual">Manual runs</option><option value="scheduled">Scheduled + manual</option></select></Field><Field label="Approval policy"><select value={form.approval_policy} onChange={(event) => update('approval_policy', event.target.value as Settings['approval_policy'])}><option value="review_first">Review every batch</option><option value="automatic">Automatic when truth-checked</option></select></Field><Field label="Lead target"><input min="1" max="25" type="number" value={form.lead_target} onChange={(event) => update('lead_target', Number(event.target.value))} /></Field><Field label="Daily email cap"><input min="0" max="25" type="number" value={form.email_cap} onChange={(event) => update('email_cap', Number(event.target.value))} /></Field><Field label="Daily application cap"><input min="0" max="10" type="number" value={form.application_cap} onChange={(event) => update('application_cap', Number(event.target.value))} /></Field><Field label="Schedule hour"><input min="0" max="23" type="number" value={form.schedule_hour} onChange={(event) => update('schedule_hour', Number(event.target.value))} /></Field><Field label="Timezone"><input value={form.timezone} onChange={(event) => update('timezone', event.target.value)} /></Field><Field label="Follow-up days"><input value={form.followup_days} onChange={(event) => update('followup_days', event.target.value)} placeholder="3,6,10,15,21,30,45" /></Field><Field label="Minimum compensation"><input value={form.min_compensation} onChange={(event) => update('min_compensation', event.target.value)} placeholder="Optional" /></Field></div></section>
