...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      {data && approvalIntent ? <ApprovalModal items={data.approvals.filter((item) => item.status === 'pending' && item.readiness === 'ready' && (approvalIntent === 'all' || item.item_type === approvalIntent))} scope={approvalIntent} busy={busy} onCancel={() => setApprovalIntent(null)} onConfirm={() => void approveAndRun()} /> : null}
    </main>
  );
}
...
function ApprovalModal({ items, scope, busy, onCancel, onConfirm }: { items: ApprovalItem[]; scope: 'email' | 'application' | 'all'; busy: boolean; onCancel: () => void; onConfirm: () => void }) {
